...
filterBtnsContainer.addEventListener("click", (e) => {
    ...
    !e.target.classList.contains("active")) {
        ...
        document.querySelector(".filter-status p").innerHTML = `filtrando temas sobre <span>${e.target.innerHTML}</span>`;
        setTimeout(() => {
...
function portfolioItemDetails() {
    ...
    document.querySelector(".pp-counter").innerHTML = `${portfolioItemIndex+1} de ${portfolioItems.length} ( <span title="category">${document.querySelector(".portfolio-filter-btn.active").innerHTML}</span> )`;
}
...
function changePortfolioItem(dir) {
    if (dir == "prev") {
        portfolioItemIndex--;
    } else {
        portfolioItemIndex++;
